                                   ...
                                   int               ngrow)
{
    ...
    if (level <  parent->finestLevel())
    {
        ...
        for (int i = 0; i < Nyx::theVirtualParticles().size(); i++)
        {
            ...
            if(Rhs.DistributionMap()==particle_mf.DistributionMap() && 
                    particle_mf.boxArray().CellEqual(Rhs.boxArray())) //ACJ
                MultiFab::Add(Rhs, particle_mf, 0, 0, 1, 0);
            else                                //ACJ
                Rhs.ParallelAdd(particle_mf);   //ACJ
...
Gravity::AddVirtualParticlesToRhs(int finest_level, const Vector<MultiFab*>& Rhs_particles)
{
    ...
    if (finest_level < parent->finestLevel())
    {
        ...
        for (int i = 0; i < Nyx::theGhostParticles().size(); i++)
        {
            ...
            if((*Rhs_particles[finest_level]).DistributionMap()==VirtPartMF.DistributionMap() && 
                    (*Rhs_particles[finest_level]).boxArray().CellEqual(VirtPartMF.boxArray())) //ACJ
                                 ...
                                 MultiFab&         Rhs)
{
    ...
    if (level > 0)
    {
        ...
        for (int i = 0; i < Nyx::theGhostParticles().size(); i++)
        {
            ...
            if(Rhs.DistributionMap()==ghost_mf.DistributionMap() && 
                    Rhs.boxArray().CellEqual(ghost_mf.boxArray()))  //ACJ
...
Gravity::AddGhostParticlesToRhs(int level, const Vector<MultiFab*>& Rhs_particles)
{
    ...
    if (level > 0)
    {
        ...
        for (int i = 0; i < Nyx::theGhostParticles().size(); i++)
        {
            ...
            if((*Rhs_particles[0]).DistributionMap()==GhostPartMF.DistributionMap() &&
                 (*Rhs_particles[0]).boxArray().CellEqual(GhostPartMF.boxArray()))  //ACJ
                MultiFab::Add(*Rhs_particles[0], GhostPartMF, 0, 0, 1, 0);
            else                                            //ACJ
                Rhs_particles[0]->ParallelAdd(GhostPartMF); //ACJ
        }
    }
    amrex::Gpu::Device::streamSynchronize();
}
