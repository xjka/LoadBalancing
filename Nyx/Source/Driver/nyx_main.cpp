        ...
        std::vector<float> &z)
{
    ...
    rl_outfile.write((char*)steps.data(), sizeof(int)*steps.size()); 
    rl_outfile.write((char*)z.data(), sizeof(float)*z.size()); 
    for(auto & ranks : rank_loads){ 
        long numranks = ranks.size(); 
        rl_outfile.write((char*)&numranks, sizeof(long));
        rl_outfile.write((char*)ranks.data(), sizeof(long)*numranks); 
    } 
    rl_outfile.close(); 
}
//ACJ
...
nyx_main (int argc, char* argv[])
{
    ...
    BL_PROFILE_VAR_NS("nyx_main::LoadBalanceProfiling()", loadBalanceProfiling); //ACJ
    std::vector<amrex::Vector<long>> rank_loads; //ACJ
    std::vector<int> rl_steps; //ACJ
    std::vector<float> rl_z; //ACJ
            ...
            if(current_step%dualGridProfileFreq==0 or current_step%loadBalanceInt==0 or current_step%loadBalanceInt==loadBalanceInt-1)
            {
                ...
                if(ParallelDescriptor::IOProcessor())
                {
                    ...
                    if(current_step%checkInt==0 or current_step%loadBalanceInt==0){ //write output whenever we write a checkpoint
                        write_rank_loads(Nyx::dual_grid_load_balance, rank_loads, rl_steps, rl_z);
                    }
                }
                BL_PROFILE_VAR_STOP(loadBalanceProfiling);
                //amrex::Print()<<"Tiny Profiling Output:"<<std::endl;
