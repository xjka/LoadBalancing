...
c ::: --------------------------------------------------------------
      subroutine cartgridminmax3d (data, dlo1, dlo2, dlo3, dhi1,dhi2, dh
     &i3, vfracdata, vfeps,dmin, dmax)
      implicit none

      integer dlo1, dlo2, dlo3, dhi1, dhi2, dhi3
      DOUBLE PRECISION data(dlo1:dhi1 ,dlo2:dhi2, dlo3:dhi3)
      DOUBLE PRECISION vfracdata(dlo1:dhi1 ,dlo2:dhi2, dlo3:dhi3)
...
c ::: --------------------------------------------------------------
      subroutine cinterp3d (fine,floi1,floi2,floi3,fhii1,fhii2,fhii3,fbl
     &o,fbhi,nvar,lratio, crse,clo,chi,cblo,cbhi,fslo,fshi, cslope,cle
     &n,fslope,fdat,flen,voff)
      ...
      DOUBLE PRECISION fine(floi1:fhii1,floi2:fhii2,floi3:fhii3,nvar)
...
c ::: local var
      ...
      DOUBLE PRECISION xoff, yoff, zoff
...
c ::: ::::: compute slopes in z direction
          do 160 i = 1, clen
              cen = 0.5D0*(crse(i+kst,n)-crse(i-kst,n))
              forw = crse(i+kst,n)-crse(i,n)
              back = crse(i,n)-crse(i-kst,n)
              slp = sign(1.0D0,cen)*min(abs(cen),abs(forw),abs(back))
              cslope(i,3)=merge(slp,0.0D0,forw*back>=0.0d0)
160       continue
...
c ::: ..,.......::::: strip out a fine grid slope vector
                      do 190 ioff = 1, lratio
                          icc = clo + ist + jst*(jc-jslo) + kst*(kc-kslo
     &)
                          istart = ioff
...
c                                 --- break ---
                              ...
                              do 230 i = fblo(1), fbhi(1)
                                  fn = i-fslo(1)+1
                                  fine(i,j,k,n) = fdat(fn) + volratio*(v
     &off(fn)*fslope(fn,1)+yoff*fslope(fn,
     &2)+ zoff*fslope(fn,3))
230                           continue
220                       continue
221                       continue
...
c ::: ..,.......::::: strip out a fine grid slope vector
                      do 260 joff = 1, lratio
                          icc = clo + ist*(ic-islo) + jst + kst*(kc-kslo
     &)
                          istart = joff
...
c                                 --- break ---
                              ...
                              do 300 j = fblo(2), fbhi(2)
                                  fn = j-fslo(2)+1
                                  fine(i,j,k,n) = fdat(fn) + volratio*(x
     &off*fslope(fn,1)+voff(fn)*fslope(fn,
     &2)+ zoff*fslope(fn,3))
300                           continue
290                       continue
291                       continue
...
c ::: ..,.......::::: strip out a fine grid slope vector
                      do 330 koff = 1, lratio
                          icc = clo + ist*(ic-islo) + jst*(jc-jslo) +kst
                          istart = koff
...
c                                 --- break ---
                              ...
                              do 370 k = fblo(3), fbhi(3)
                                  fn = k-fslo(3)+1
                                  fine(i,j,k,n) = fdat(fn) + volratio*(x
     &off*fslope(fn,1)+yoff*fslope(fn,2)+v
     &off(fn)*fslope(fn,3))
370                           continue
360                       continue
361                       continue
...
c ::: --------------------------------------------------------------
      subroutine pcinterp3d (fine,floi1,floi2,floi3,fhii1,fhii2,fhii3,fb
     &lo,fbhi,lrat,nvar,crse,cloi1,cloi2,cloi3,chii1,chii2,chii3,cblo,
     &cbhi,temp,tloi,thii)
      ...
      DOUBLE PRECISION fine(floi1:fhii1,floi2:fhii2,floi3:fhii3, nvar)
      DOUBLE PRECISION crse(cloi1:chii1,cloi2:chii2,cloi3:chii3, nvar)
...
c ::: --------------------------------------------------------------
      subroutine vcinterp3d (fine,floi1,floi2,floi3,fhii1,fhii2,fhii3,fb
     &loi1,fbloi2,fbloi3, fbhii1,fbhii2,fbhii3,nvar,lratio,crse,clo,ch
     &i,cblo,cbhi, fslo,fshi,cvloi1,cvloi2,cvloi3,cvhii1,cvhii2,cvhii3
     &, cslope,clen,fslope,fdat,flen,voff, cvc1,fvc1,cvc2,fvc2,cvc3,fv
     &c3)
      ...
      DOUBLE PRECISION cvc1(cvloi1 :cvhii1+1)
      DOUBLE PRECISION cvc2(cvloi2 :cvhii2+1)
      DOUBLE PRECISION cvc3(cvloi3 :cvhii3+1)
      DOUBLE PRECISION fvc1(fbloi1 :fbhii1+1)
      DOUBLE PRECISION fvc2(fbloi2 :fbhii2+1)
      DOUBLE PRECISION fvc3(fbloi3 :fbhii3+1)
...
c ::: local var
      ...
      if (maxlen .eq. lenx) then
          do 380 i = fbloi1, fbhii1
              fn = i-fslo(1)+1
              ic = (i+lratio*iabs(i))/lratio-iabs(i)
              fcen = 0.5D0*(fvc1(i)+fvc1(i+1))
              ccen = 0.5D0*(cvc1(ic)+cvc1(ic+1))
              voff(fn) = (fcen-ccen)/(cvc1(ic+1)-cvc1(ic))
380       continue
      else if (maxlen .eq. leny) then
          do 390 j = fbloi2, fbhii2
              fn = j-fslo(2)+1
              jc = (j+lratio*iabs(j))/lratio-iabs(j)
              fcen = 0.5D0*(fvc2(j)+fvc2(j+1))
              ccen = 0.5D0*(cvc2(jc)+cvc2(jc+1))
              voff(fn) = (fcen-ccen)/(cvc2(jc+1)-cvc2(jc))
390       continue
      else
          do 400 k = fbloi3, fbhii3
              fn = k-fslo(3)+1
              kc = (k+lratio*iabs(k))/lratio-iabs(k)
              fcen = 0.5D0*(fvc3(k)+fvc3(k+1))
              ccen = 0.5D0*(cvc3(kc)+cvc3(kc+1))
...
c                                 --- break ---
                              ...
                              do 510 i = fbloi1, fbhii1
                                  fn = i-fslo(1)+1
                                  fine(i,j,k,n) = fdat(fn) + voff(fn)*fs
     &lope(fn,1)+yoff*fslope(fn,2)+ zoff*f
     &slope(fn,3)
510                           continue
500                       continue
501                       continue
...
c                                 --- break ---
                              ...
                              do 580 j = fbloi2, fbhii2
                                  fn = j-fslo(2)+1
                                  fine(i,j,k,n) = fdat(fn) + xoff*fslope
     &(fn,1)+voff(fn)*fslope(fn,2)+zoff*fs
     &lope(fn,3)
580                           continue
570                       continue
571                       continue
...
c                                 --- break ---
                              ...
                              do 650 k = fbloi3, fbhii3
                                  fn = k-fslo(3)+1
                                  fine(i,j,k,n) = fdat(fn) + xoff*fslope
     &(fn,1)+yoff*fslope(fn,2)+voff(fn)*fs
     &lope(fn,3)
650                           continue
640                       continue
641                       continue
